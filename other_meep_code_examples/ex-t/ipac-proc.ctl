...
(define-param sx (+ 10 dpml))
(define-param sy (+ 5 dpml))
(define-param lamb 1)
(define-param inci 1.5)
...
(geometric-object-duplicates (vector3 pit 0) 0 20
(make block (center (/ pit 1.5) (/ dep 2)) (size (/ pit 2) dep ) (material air)))
(geometric-object-duplicates (vector3 (- 0 pit) 0 ) 0 20
(make block (center (/ pit 1.5) (/ dep 2)) (size (/ pit 2) dep) (material air)))
))


(define-param rotation-angle (* (/ 45 360) 2 pi)) ; 傾ける角度
(define-param beam-waist 1) ; 光源の分布幅
(define-param source-points 300) ; 点光源の数多めに設定
(define-param source-size (* 3 beam-waist)) ; 光源サイズ分布幅より大
...
(set! resolution 20) ;; initially 10
(use-output-directory)
(run-until (* 6 sx)
	   (at-beginning output-epsilon)
	   (to-appended "ex" (at-every (/ (* 0.398 pit) 4)  output-efield-x ))
